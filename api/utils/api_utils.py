...
def add_tenant_id_to_kwargs(func):
    ...
    Resolution order:
      1. ``X-Tenant-Id`` request header – validated against the user's memberships.
      2. Auto-resolve from the user's ``UserTenant`` relations (OWNER > ADMIN > NORMAL).
      3. Legacy fallback: ``current_user.id`` for single-tenant deployments.
    """
    @wraps(func)
