...
class KnowledgebaseService(CommonService):
    ...
    def _visibility_and_status_filter(cls, joined_tenant_ids, user_id):
        ...
        Visibility rules (team-based permission model):
        - All KBs whose tenant_id is in the user's accessible tenant list
    ...
    def accessible(cls, kb_id, user_id):
        ...
        In the team-based permission model, any team member can access
        a dataset that belongs to one of their teams.

        Args:
            kb_id: Knowledge base ID
            user_id: User ID (not tenant_id)
        """
        ...
        # Check if the user is a member of the team that owns this KB
        ut = UserTenantService.filter_by_tenant_and_user_id(kb.tenant_id, user_id)
        return ut is not None and ut.role != UserTenantRole.INVITE

    @classmethod
    @DB.connection_context()
    def accessible_by_tenant(cls, kb_id, tenant_id):
        """Check if a dataset is accessible from a tenant context.

        Used by API layer where tenant_id (team ID) is already resolved.
        """
        e, kb = cls.get_by_id(kb_id)
        if not e:
            return False
        if kb.status != StatusEnum.VALID.value:
            return False
        # KB must belong to this tenant (team)
        return kb.tenant_id == tenant_id

    @classmethod
    @DB.connection_context()
    def get_kb_by_id(cls, kb_id, tenant_id):
        """Get dataset by ID, scoped to the given team."""
        e, kb = cls.get_by_id(kb_id)
        if not e or not cls.accessible_by_tenant(kb_id, tenant_id):
            return []
        return [kb.to_dict()]

    @classmethod
    @DB.connection_context()
    def get_kb_by_name(cls, kb_name, tenant_id):
        """Get dataset by name, scoped to the given team."""
        kbs = cls.query(name=kb_name, status=StatusEnum.VALID.value)
        for kb in kbs:
            if cls.accessible_by_tenant(kb.id, tenant_id):
                return [kb.to_dict()]
        return []
