...
class UserCanvasService(CommonService):
    ...
    def accessible(cls, canvas_id, tenant_id):
        """Check if a canvas is accessible from the given team context.

        Args:
            canvas_id: The canvas ID
            tenant_id: The resolved Tenant/Team ID from X-Tenant-Id header
        """
        from api.utils.permission import is_superadmin
        ...
        # If the canvas belongs to a user who is in this tenant/team, it's accessible
        canvas_owner_user_id = c["user_id"]
        # Check if the canvas owner is a member of this team via direct DB query
        with DB.connection_context():
            ut = UserTenant.select().where(
                UserTenant.tenant_id == tenant_id,
                UserTenant.user_id == canvas_owner_user_id
            ).first()
        if ut is not None and ut.role != UserTenantRole.INVITE:
            return True

        # Also check the requesting user directly
        # Need current_user – we rely on the fact that add_tenant_id_to_kwargs
        # has already validated the user's membership in this team
        # So if the canvas's tenant_id matches our resolved tenant_id, it's accessible
        # For canvases created under the current team's users
        return False
