...
from api.utils.permission import team_admin_required, is_team_admin, is_team_member, is_superadmin
...
def _require_canvas_admin_async(func):
    """Decorator: require team-admin-or-higher permission for canvas write operations.

    Resolves the team that owns the canvas and checks the current user's role
    in that team.
    """
    @wraps(func)
    async def wrapper(*args, **kwargs):
        agent_id = kwargs.get('agent_id')
        tenant_id = kwargs.get('tenant_id')  # resolved team ID from add_tenant_id_to_kwargs

        # First, verify the canvas is accessible
        if not await thread_pool_exec(UserCanvasService.accessible, agent_id, tenant_id):
            return get_json_result(data=False, message="No permission to access this agent.", code=RetCode.PERMISSION_ERROR)

        # For write operations, require team-admin level in the resolved team
        # Use current_user directly (already imported via api.apps)
        if current_user and current_user.id and tenant_id:
            from api.utils.permission import get_user_role_in_team
            role = get_user_role_in_team(current_user.id, tenant_id)
            if role not in ("superadmin", "owner", "admin"):
                return get_json_result(data=False, message="Write permission denied. Team admin or higher role required.", code=RetCode.PERMISSION_ERROR)
        else:
            return get_json_result(data=False, message="Authentication required.", code=RetCode.PERMISSION_ERROR)
...
def list_agents(tenant_id):
    ...
    from api.utils.permission import get_user_accessible_tenant_ids
    authorized_owner_ids = set(get_user_accessible_tenant_ids(tenant_id))
...
def list_agent_tags(tenant_id):
    ...
    from api.utils.permission import get_user_accessible_tenant_ids
    canvas_category = request.args.get("canvas_category")
    joined_ids = list(get_user_accessible_tenant_ids(tenant_id))
...
async def create_agent(tenant_id):
    ...
    owner_nickname = _get_user_nickname(current_user.id)
