...
from api.utils.permission import is_team_admin, resolve_tenant_id
...
def token_list():
    ...
    try:
        ...
        # Verify admin access for the resolved tenant
        if not is_team_admin(current_user.id, tenant_id):
            return get_json_result(data=False, message="Only team admin or higher can manage API tokens.", code=RetCode.PERMISSION_ERROR)
...
def new_token():
    ...
    try:
        ...
        if err:
            return get_data_error_result(message=err)
        if not is_team_admin(current_user.id, tenant_id):
            return get_json_result(data=False, message="Only team admin or higher can create API tokens.", code=RetCode.PERMISSION_ERROR)
