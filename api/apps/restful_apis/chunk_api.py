...
async def parse(tenant_id, dataset_id):
    if not KnowledgebaseService.accessible_by_tenant(kb_id=dataset_id, tenant_id=tenant_id):
...
async def retrieval_test(tenant_id):
    ...
    for id in kb_ids:
        if not KnowledgebaseService.accessible_by_tenant(kb_id=id, tenant_id=tenant_id):
