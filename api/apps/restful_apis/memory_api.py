...
from api.utils.api_utils import validate_request, get_request_json, get_error_argument_result, get_json_result, add_tenant_id_to_kwargs
from api.apps.services import memory_api_service
from api.utils.pagination_utils import validate_rest_api_page_size
from api.utils.permission import team_admin_required
...
@team_admin_required
async def create_memory(tenant_id):
    ...
    try:
        ...
        success, res = await memory_api_service.create_memory(memory_info, tenant_id=tenant_id)
        if timing_enabled:
            logging.info(
...
@add_tenant_id_to_kwargs
@team_admin_required
async def update_memory(memory_id, tenant_id):
    req = await get_request_json()
...
@add_tenant_id_to_kwargs
@team_admin_required
async def delete_memory(memory_id, tenant_id):
    try:
...
@login_required
@add_tenant_id_to_kwargs
async def list_memory(tenant_id):
    filter_params = {
