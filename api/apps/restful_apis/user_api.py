...
def rollback_user_registration(user_id):
    # Clean up all tenant associations for this user
    try:
        for ut in UserTenantService.query(user_id=user_id):
            try:
                TenantService.delete_by_id(ut.tenant_id)
            except Exception:
                pass
            try:
                TenantLLM.delete().where(TenantLLM.tenant_id == ut.tenant_id).execute()
            except Exception:
                pass
            try:
                UserTenantService.delete_by_id(ut.id)
            except Exception:
                pass
    except Exception:
        pass
...
def user_register(user_id, user):
    user["id"] = user_id
    # Generate an independent tenant_id so multiple teams can be created later
    tenant_id = get_uuid()
    tenant = {
        "id": tenant_id,
    ...
    tenant_llm = get_init_tenant_llm(tenant_id)
...
async def user_add():
    ...
    try:
        ...
        user = users[0]
        # Only login as the new user if no one is currently authenticated,
        # so that an already-logged-in user (e.g. superadmin registering users)
        # does not get their session replaced.
...
async def tenant_info():
    """
    Get tenant information for the currently active team.
    ---
    tags:
      - Tenant
    security:
    ...
    """
    from api.utils.permission import resolve_tenant_id
    try:
        tenant_id, err = resolve_tenant_id()
        if err:
        ...
        data = tenant.to_dict()
        # Rename id → tenant_id for frontend compatibility
        data["tenant_id"] = data.pop("id")
        # Enrich with user role
        from api.utils.permission import get_user_role_in_team
        data["role"] = get_user_role_in_team(current_user.id, tenant_id)
...
async def set_tenant_info():
    ...
    try:
        ...
        from api.utils.permission import is_team_admin
        if not is_team_admin(current_user.id, tid):
            return get_json_result(
                data=False,
                message="Only team admin or higher can update team settings.",
