...
from api.utils.permission import team_admin_required, is_team_member, is_team_admin, is_superadmin
...
async def _ensure_accessible_chat(chat_id):
    """Check the current user can *read* the chat (any team member)."""
...
async def _ensure_admin_chat(chat_id):
    """Check the current user has admin-level access to the chat's team."""
    chats = await thread_pool_exec(
        DialogService.query, id=chat_id, status=StatusEnum.VALID.value
    )
    if not chats:
        return None
    tenant_id = chats[0].tenant_id
    if is_team_admin(current_user.id, tenant_id):
        return chats
    return None
...
async def _validate_dataset_ids(dataset_ids, tenant_id):
    ...
    for dataset_id in normalized_ids:
        if not await thread_pool_exec(KnowledgebaseService.accessible_by_tenant, kb_id=dataset_id, tenant_id=tenant_id):
...
async def list_chats():
    ...
    try:
        ...
        authorized_owner_ids = set(get_user_accessible_tenant_ids(current_user.id))
...
async def get_chat(chat_id):
    try:
        if not await _ensure_accessible_chat(chat_id):
            return get_json_result(
                data=False,
...
async def update_chat(chat_id):
    if not await _ensure_admin_chat(chat_id):
        return get_json_result(
            data=False, message="No permission to update this chat.", code=RetCode.PERMISSION_ERROR
        )
...
async def delete_chat(chat_id):
    if not await _ensure_admin_chat(chat_id):
        return get_json_result(
            data=False, message="No permission to delete this chat.", code=RetCode.PERMISSION_ERROR
        )
...
async def bulk_delete_chats():
    ...
    for chat_id in unique_ids:
        if not await _ensure_admin_chat(chat_id):
            errors.append(f"Chat({chat_id}) not found or no permission.")
            continue
...
async def create_session(chat_id):
    """Create a new conversation session.  Any team member can create sessions."""
    if not await _ensure_accessible_chat(chat_id):
...
async def update_message_feedback(chat_id, session_id, msg_id):
    owned = await _ensure_accessible_chat(chat_id)
    if not owned:
        return get_json_result(data=False, message="No authorization.", code=RetCode.AUTHENTICATION_ERROR)
    try:
        req = await get_request_json()
