...
def get_dataset(dataset_id: str, tenant_id: str):
    ...
    if not KnowledgebaseService.accessible_by_tenant(dataset_id, tenant_id):
...
def list_datasets(tenant_id: str, args: dict):
    ...
    if ext_fields.get("owner_ids", []):
        ...
        # In team-based model, the team (tenant_id) is already resolved;
        # list datasets belonging to this team.
...
async def search_datasets(tenant_id: str, req: dict):
    ...
    for kb_id in kb_ids:
        if not KnowledgebaseService.accessible_by_tenant(kb_id, tenant_id):
