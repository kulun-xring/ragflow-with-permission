...
def _joined_tenant_ids(user_id: str) -> set[str]:
    ...
    return {t["tenant_id"] for t in user_tenants if t.get("role") not in (None, UserTenantRole.INVITE)}


def _memory_accessible(memory) -> bool:
    """In team-based permission model, any team member can access memory in their teams."""
    from api.utils.permission import is_team_member, is_superadmin
    if is_superadmin(current_user.id):
        return True
    return is_team_member(current_user.id, memory.tenant_id)
...
async def create_memory(memory_info: dict, tenant_id: str = None):
    """
    :param memory_info: {
        "name": str,
    ...
    :param tenant_id: Team (tenant) ID to create the memory under
    """
    # check name length
